...
- `HN_Onc_Digest_2026-08-06.html` — the current/live digest cycle. Each cited study is a native `<details>` card with its own "Key conclusion" and "Counterpoint." Seven high-value findings also contain source-grounded evidence briefs with tables, small charts, protocol maps, deeper interpretation, and—where the license permits—an attributed article figure. Wide screens turn that source into a master-detail workspace: an interactive Digest navigator on the left and a generous reader on the right, with section/finding navigation, persistent reviewed state, next-unread navigation, and copy controls. A persistent `Aa` palette adds System, Light, Sepia, and Dark reading themes plus four text-size steps. Mobile inline expansion and a fully-expanded tumor-board printout remain built in.
