...
1. Search from the prior digest's cutoff through the current run date (normally about 12-17 days). Use the saved cutoff rather than an arbitrary lookback so publication delays and a late run do not create gaps. Prioritize major journals: J Clin Oncol, JAMA Oncology, JAMA Otolaryngology-Head & Neck Surgery, Laryngoscope, Lancet, Lancet Oncology, NEJM, Int J Radiat Oncol Biol Phys, Head & Neck, Oral Oncology, Ann Surg Oncol, Clin Cancer Res, Annals of Oncology.
...
- Every reference must include a nested `.evidence-brief`, with depth proportional to the available source. For a primary study, cover design, population, intervention/exposure, endpoint, exact results, clinical interpretation, and limitations. For a commentary, guideline, review, correspondence, or registry-only item, explain its scope, argument or protocol, what it adds, and what it cannot establish; never manufacture patient-level evidence to make a thin source look richer. A useful brief can include a provenance bar, concise methods/deck paragraph, exact metric cards, an accessible table, an original small chart, a trial-arm/pathway map, and paired "Clinical read" / "Do not overread" analysis.
...
- Carry the substantive scope or methods, exact results where available, interpretation, limitations, and figure/source link from every evidence brief into the Markdown fallback as indented prose. The `.md` does not need to reproduce the visual card, chart, or protocol diagram, but it must not lose their informational content.
...
- Persist only stable reviewed reference IDs in `localStorage`, keyed by digest title. Storage failures (including restrictive local-file contexts) must fail silently while retaining in-session behavior. Never store medical content, browsing history, or user-authored notes.
...
- Native `<details>` remains the required foundation and fallback. The generated navigator/reader are hidden on smaller screens and in print. Escape returns the wide reader to the cycle overview.
- In the wide master-detail workspace, keep both columns stationary within the viewport and let the right reader scroll independently. Moving between findings resets only the right reader to its top; it must not scroll or reposition the whole page. Use the selected finding's own conclusion as the prominent reader title and keep the recurring section title in the smaller context line so successive articles do not appear to repeat the same heading.
- At 640px and below, optimize for a one-handed clinical scan: keep the dated title, window, and cycle-identity statement visible; collapse methodology/sources/how-to text behind one native disclosure; turn the topic pills into a sticky horizontally scrollable rail; show a compact reviewed-progress card with **Next unread**; and collapse the Highlights list behind a clearly labeled two-minute-overview disclosure. Do not remove or summarize away evidence.
- Mobile **Next unread** opens one full finding, marks it reviewed, closes any previously open finding, and scrolls the selected card below the sticky topic rail. Directly opening a finding follows the same one-card-at-a-time behavior. Progress uses the same stable reference IDs and storage key as desktop.
- Mobile touch targets are at least 40px tall, with primary persistent controls such as `Aa` at least 44×44px. The document itself must never overflow horizontally at 320–430px; wide evidence tables may scroll inside their own bordered container.
...
- **Layout**: page max-width 1500px with 40px desktop gutters; a left-side navigator and right-side reader in a 1:2 ratio, separated by a 28-48px gap. The navigator has a 380px minimum at the wide breakpoint. Center the masthead in a 940px measure above both panes. At 1319px and below, collapse to the native one-column digest with an 880px maximum reading width. At 640px and below, use 16px gutters and the compact mobile rhythm.
- **Mobile scan path**: the first phone viewport should reach the cycle navigation/progress and expose the first clinical section without forcing the reader through provenance boilerplate. Preserve the compact methods disclosure, horizontal sticky topic rail, reviewed-progress/Next-unread card, collapsible key-takeaways card, one-open-finding behavior, 68px finding scroll margin, and internally scrollable evidence tables from the current file.
- **Palette/tokens**: copy all current `:root` variables exactly, including the reader type-size tokens and manual theme overrides. Light mode uses ink `#17212b`, deep editorial blue `#174f78`, high-chroma link blue `#075f9a`, off-white canvas `#fcfcfb`, and cool gray surfaces/borders. Sepia uses warm link brown `#8a430f`; dark mode uses bright link blue `#72c7f2`. Hyperlinks must remain identifiable without hover or color perception alone: medium weight with a persistent 1.5px underline that strengthens on hover. Practice-changing uses the blue-tinted `--success-*` pair; hypothesis-generating uses muted amber `--warning-*`; background stays neutral. Section counterpoints use a restrained warm tint; Go deeper uses a cool blue tint.
...
- **Print support**: `beforeprint`/`afterprint` listeners temporarily force all `details.ref` open and then restore the prior open state; the `@media print` block hides the reader workspace and appearance control and forces the printable source back to the light palette regardless of the current screen theme.
